...
function Button({ className = "", variant, children, ...props }) {
  ...
  return <button className={`${base} px-4 py-2 text-sm font-semibold transition disabled:opacity-50 ${className}`} {...props}>{children}</button>;
}

function Input({ className = "", ...props }) {
  return <input className={`rounded-xl border border-blue-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 ${className}`} {...props} />;
}
...
function StatCard({ label, value }) {
  return <div className="rounded-2xl bg-white p-4 shadow-sm border border-blue-100"><p className="text-sm text-blue-700">{label}</p><p className="text-2xl font-bold text-blue-950">{value}</p></div>;
}

function SmallNumberInput({ value, onChange, width = "w-16", autoAdvance = false, colIndex }) {
...
function AppCard({ children, className = "" }) {
  return <Card className={`rounded-2xl border border-blue-300 bg-white/95 shadow-md backdrop-blur ${className}`}>{children}</Card>;
}
      ...
      <CardContent className={bigScreen ? "p-8" : "p-5"}>
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-blue-700">Bracket mode highlights bye positions. Eliminator mode highlights the top 4. Bubble position is highlighted amber.</p>
          </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-blue-200 bg-white">
          <table className={bigScreen ? "w-full text-2xl" : "w-full text-sm"}>
            <thead className="bg-blue-800 text-white">
              <tr>
                <th className="p-3 text-left">Rank</th>
                <th className="p-3 text-left">Bowler</th>
                <th className="p-3 text-right">Scratch</th>
                {useHandicapScores && <th className="p-3 text-right">Hdcp Total</th>}
                ...
                return (
                  <React.Fragment key={`${b.seed}-${b.name}`}>
                    ...
                    <tr className={rowClass(b)}>
                      <td className="p-3 font-black">{b.rank}</td>
                      <td className="p-3 font-semibold">{b.name}</td>
                      <td className="p-3 text-right">{b.scratch}</td>
                      {useHandicapScores && <td className="p-3 text-right font-semibold">{b.handicap}</td>}
                      <td className={`p-3 text-right font-black ${diff === null ? "" : diff >= 0 ? "text-green-700" : "text-red-600"}`}>{diff === null ? "—" : `${diff >= 0 ? "+" : ""}${diff}`}</td>
                      <td className="p-3 text-right">{statusBadge(b)}</td>
                    </tr>
                  </React.Fragment>
                );
              })}
...
function PlaceholderTab({ title, note }) {
  return <AppCard><CardContent className="p-5"><h2 className="text-xl font-semibold text-blue-900">{title}</h2><p className="mt-2 text-sm text-blue-700">{note}</p></CardContent></AppCard>;
}
